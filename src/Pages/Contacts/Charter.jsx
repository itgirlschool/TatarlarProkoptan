...
export default function Charter({ isOpen }) {
  ...
  const onHandleCommon = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isCommonOpen: !isChapterOpen.isCommonOpen,
    });
  };
  const onHandleGoals = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isGoalsOpen: !isChapterOpen.isGoalsOpen,
    });
  };
  const onHandleRights = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isRightsOpen: !isChapterOpen.isRightsOpen,
    });
  };
  const onHandleMembers = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isMembersOpen: !isChapterOpen.isMembersOpen,
    });
  };
  const onHandleControls = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isControlsOpen: !isChapterOpen.isControlsOpen,
    });
  };
  const onHandlePropety = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isPropetyOpen: !isChapterOpen.isPropetyOpen,
    });
  };
  const onHandleReorganisation = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isReorganisationOpen: !isChapterOpen.isReorganisationOpen,
    });
  };
  const onHandleChanges = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isChangesOpen: !isChapterOpen.isChangesOpen,
    });
  };
  const onHandleFinal = () => {
    setIsChapterOpen({
      ...isChapterOpen,
      isFinalOpen: !isChapterOpen.isFinalOpen,
    });
  };
  ...
  return (
    <section className={style.document}>
      ...
      <div className={style.chapter}>
        ...
        {isChapterOpen.isGoalsOpen && (
          <ol>
            ...
            <li className={style.chapter__numberitem}>
              Для выполнения своих целей Автономия ставит перед собой следующие
              задачи:
              <ul>
                {tasks.map((item) => (
                  <li key={item.id} className={style.chapter__item}>
        ...
        {isChapterOpen.isRightsOpen && (
          <ol>
            ...
            <li className={style.chapter__numberitem}>
              ...
              национально-территориальное самоопределение. Осуществление права
              на татарскую национально-культурную автономию не должно наносить
              ущерб интересам других этнических общностей Кемеровской области.
            </li>
            <li className={style.chapter__numberitem}>
              Автономия обязана:
              <ul>
        ...
        {isChapterOpen.isMembersOpen && (
          <ol>
            <li className={style.chapter__numberitem}>
              ...
              общности, постоянно проживающие на территории Российской Федерации
              и признающие Устав Автономии, способствующие своей деятельностью
              осуществлению целей Автономии. Члены Автономии имеют равные права
              и равные обязанности.
            </li>
            <li className={style.chapter__numberitem}>
              Членство в Автономии осуществляется на добровольных началах. Приём
              в члены Автономии производится в индивидуальном порядке, на
              основании письменного заявления физического лица и решения
              уполномоченного руководящего органа юридического лица, поданного в
              Правление Автономии. Правление Автономии на заседании выносит
              решение о принятии заявителя в члены Автономии или обоснованное
              решение об отказе в приёме. Заявитель становится Членом Автономии
              с момента принятия положительного решения Правления Автономии.
            </li>
            <li className={style.chapter__numberitem}>
            ...
            <li className={style.chapter__numberitem}>
              ...
              заявления (решения) в Правление Автономии. При выходе из Автономии
              вступительные и членские взносы не возвращаются.
            </li>
            <li className={style.chapter__numberitem}>
              Решение об исключении из членов Автономии и прекращении членства
              принимается Правлением Автономии. Решение считается принятым, если
              за него проголосовало большинство голосов членов Правления,
          ...
          <ol>
            <li className={style.chapter__numberitem}>
              ...
              <ul>
                ...
                <li className={style.chapter__item}>Председатель Правления;</li>
                <li className={style.chapter__item}>Ревизор.</li>
              </ul>
            </li>
            <li className={style.chapter__numberitem}>
              Высшим органом управления Автономии является Общее собрание членов
              Автономии (далее Общее собрание), созываемое не реже одного раза в
              год.
            </li>
            <li className={style.chapter__numberitem}>
            ...
            <li className={style.chapter__numberitem}>
              Предложение о созыве внеочередного Общего собрания направляется в
              письменной форме в Правление Автономии с указанием вопросов,
            ...
            <li className={style.chapter__numberitem}>
              ...
              право выступать на Общем собрании по вопросам повестки дня, но не
            ...
            <li className={style.chapter__numberitem}>
              Для практического текущего руководства деятельностью Автономией в
              период между созывами Общих собраний избирается сроком на 5 лет
              постоянно действующий руководящий орган Автономии - Правление
              Автономии (далее -Правление).
            </li>
            <li className={style.chapter__numberitem}>
              ...
              считаются принятыми, если за них проголосовало большинство голосов
              членов Правления. Решения принимаются открытым голосованием.
            </li>
            <li className={style.chapter__numberitem}>
      ...
      <div className={style.chapter}>
        ...
        {isChapterOpen.isPropetyOpen && (
          <ol>
            ...
            <li className={style.chapter__numberitem}>
              Автономия может осуществлять предпринимательскую деятельность лишь
              постольку, поскольку это служит достижению уставных целей, ради
              которых она создана, и соответствующую этим целям. Такой
              деятельностью признаются приносящее прибыль производство товаров и
              услуг (видео и аудио материалы, организация культурно-массовых
              мероприятий и т.п.), отвечающих целям создания Автономии, а также
              приобретение и реализация ценных бумаг, участие в хозяйственных
            ...
            <li className={style.chapter__numberitem}>
              ...
              Создаваемые Автономией хозяйственные товарищества, общества и иные
              хозяйственные организации вносят в соответствующие бюджеты платежи
              в порядке и размерах, установленных законодательством РФ.
            </li>
            <li className={style.chapter__numberitem}>
              Доходы от предпринимательской деятельности Автономии не могут
              перераспределяться между членами Автономии и должны использоваться
              только для достижения уставных целей. Допускается использование
              Автономией своих средств на благотворительные цели.
            </li>
            <li className={style.chapter__numberitem}>
      ...
      <div className={style.chapter}>
        ...
        {isChapterOpen.isReorganisationOpen && (
          <ol>
      ...
      <div className={style.chapter}>
        ...
        {isChapterOpen.isChangesOpen && (
          <ol>
            ...
            <li className={style.chapter__numberitem}>
              Изменения и дополнения в Устав Автономии вступают в силу с момента
